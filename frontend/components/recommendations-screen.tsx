...
export function RecommendationsScreen({ userId }: { userId: number }) {
  ...
  useEffect(() => {
    ...
    setKValue(Number(searchParams.get("k") ?? "10") || 10);
  }, [searchParams]);
  ...
  const movies = useMemo(() => (recommendationData?.recommendations ?? []).map(buildMovieFromRecommendation), [recommendationData]);
  const availableGenres = useMemo(() => {
    const genres = new Set<string>();
    for (const movie of movies) genres.add(movie.genre);
    return ["Todas", ...Array.from(genres)];
  }, [movies]);

  const visibleMovies = useMemo(() => {
    if (selectedGenre === "Todas") return movies;
    return movies.filter((movie) => movie.genre === selectedGenre);
  }, [movies, selectedGenre]);

  useEffect(() => {
    if (!availableGenres.includes(selectedGenre)) setSelectedGenre("Todas");
  }, [availableGenres, selectedGenre]);

  function syncUrl(nextMetricId: ApiMetric) {
      ...
      {loading ? (
        ...
        <section className="mx-auto max-w-7xl px-6 pb-20 pt-8 lg:px-10">
          <div className="mb-8 flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
            <div>
              <h2 className="font-[family:var(--font-serif)] text-3xl tracking-[-0.02em]">Películas recomendadas</h2>
              <p className="mt-2 text-sm text-[var(--muted)]">
                {visibleMovies.length} resultados · {metric.name}
              </p>
            </div>
